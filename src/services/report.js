...
import { getPagerDutyActivity } from './pagerduty.js';
import { generateSummary } from './summary.js';
import { getNotesAsText, getReport, saveReport } from '../db/cache.js';
...
export async function generateReport(date, options = {}) {
  ...
  const emptyPagerDuty = {
    acknowledgedIncidents: [],
    resolvedIncidents: [],
    triggeredIncidents: [],
  };

  const [github, jira, confluence, calendar, pagerduty] = await Promise.all([
    ...
    getPagerDutyActivity(date).catch((err) => {
      // eslint-disable-next-line no-console
      console.warn(
        `[report] PagerDuty fetch failed for ${date}, using empty activity:`,
        err.message
      );
      return emptyPagerDuty;
    }),
  ]);
